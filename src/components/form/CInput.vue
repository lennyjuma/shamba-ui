...
<template>
  <div>
    <label :for="label_name" class="block text-sm/6 font-medium ">{{ label_name }}</label>
    <div class="mt-2 grid grid-cols-1">
      <input v-model="input_value" :type="input_type" :name="label_name" :id="label_name + '_id' "
             :class="[validation_error.status ? 'border-red-600 ring-red-600 focus:ring-0 outline-red-300 placeholder:text-red-600  focus:outline-red-600' : '' ,'placeholder:pl-1 col-start-1 row-start-1 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6']"
